...
CREATE TABLE scans (
    ...
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    status VARCHAR(20) DEFAULT 'queued' CHECK (status IN ('queued', 'downloading', 'scanning', 'analyzing', 'scoring', 'completed', 'failed', 'cancelled')),
    queue_position INTEGER,
    progress_percentage INTEGER DEFAULT 0,
    progress_message TEXT,
    auto_push_enabled BOOLEAN DEFAULT false,
    push_repo_url VARCHAR(500),
    push_repo_owner VARCHAR(200),
    push_repo_name VARCHAR(200),
