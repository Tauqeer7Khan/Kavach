...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    <>
      <div className="p-6">
        <div className="flex items-center gap-2">
          <KavachLogo size={28} />
          <span className="font-heading font-bold text-xl tracking-tight text-white">KAVACH</span>
          <span className="font-mono text-[10px] bg-zinc-800 text-zinc-300 px-1.5 py-0.5 rounded border border-zinc-700 mt-1">
            BETA
          </span>
        </div>
        <p className="font-mono text-xs text-zinc-500 mt-1">AI Code Security</p>
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 ${
                active
                  ? 'bg-[#7C3AED]/10 text-[#8B5CF6] border-l-2 border-[#7C3AED] shadow-[inset_0_0_20px_rgba(124,58,237,0.1)]'
      ...
      <div className="mt-auto p-6 border-t border-[#27272A]">
        <div className="bg-zinc-900/50 rounded-xl p-4 border border-zinc-800">
          <div className="flex justify-between items-center mb-3">
            <span className="font-mono text-[10px] bg-zinc-800 text-zinc-400 px-2 py-0.5 rounded uppercase tracking-wider">
              {user?.plan?.toUpperCase() || 'FREE'} PLAN
            </span>
            <span className="font-mono text-[10px] text-[#8B5CF6] hover:text-[#7C3AED] cursor-pointer">
              Upgrade
            </span>
          </div>

          <p className="font-mono text-xs text-zinc-500 mb-2">Scans this month</p>
  ...
  return (
    <div className="min-h-screen bg-[#09090B]">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#18181B] border-b border-[#27272A] p-4 flex items-center gap-4">
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          <SheetTrigger className="p-2 rounded-lg bg-[#18181B] border border-[#27272A] text-white">
            <Menu className="h-5 w-5" />
          </SheetTrigger>
          <SheetContent side="left" className="w-60 p-0 bg-[#18181B] border-r border-[#27272A] flex flex-col [&>button]:hidden">
            <SidebarContent />
          </SheetContent>
        </Sheet>
        <div className="flex items-center gap-2">
          <KavachLogo size={24} />
          <span className="font-heading font-bold text-lg tracking-tight text-white">KAVACH</span>
        </div>
      </div>

      {/* Desktop Sidebar */}
      <div className="hidden md:flex fixed left-0 top-0 h-screen w-60 z-40 bg-[#18181B] border-r border-[#27272A] flex-col">
        <SidebarContent />
      </div>

      {/* Main Content */}
      <main className="md:ml-60 w-full md:w-[calc(100%-240px)] p-4 pt-20 md:p-8 md:pt-8 min-h-screen">
        {children}
      </main>
    </div>
  );
